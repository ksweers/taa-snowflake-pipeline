...
CREATE OR REPLACE TASK TAA_DELTA_USERINFOPAYROLLMAPPING
    WAREHOUSE = &{{SNOWFLAKE_WAREHOUSE}}
    COMMENT = 'TAA delta load: USERINFOPAYROLLMAPPING'
    AFTER TAA_DELTA_ROOT
AS
CALL DELTA_LOAD_FROM_CONFIG('USERINFOPAYROLLMAPPING');

-- -----------------------------------------------------------------------------


CREATE OR REPLACE TASK TAA_DELTA_LLDETAIL
    WAREHOUSE = &{{SNOWFLAKE_WAREHOUSE}}
    COMMENT = 'TAA delta load: LLDETAIL'
    AFTER TAA_DELTA_ROOT
AS
CALL DELTA_LOAD_FROM_CONFIG('LLDETAIL');

-- -----------------------------------------------------------------------------


CREATE OR REPLACE TASK TAA_DELTA_USERINFOEMPSTATUS
    WAREHOUSE = &{{SNOWFLAKE_WAREHOUSE}}
    COMMENT = 'TAA delta load: USERINFOEMPSTATUS'
    AFTER TAA_DELTA_ROOT
AS
CALL DELTA_LOAD_FROM_CONFIG('USERINFOEMPSTATUS');


-- =============================================================================
-- FINALIZE TASK
-- Runs after all 12 leaf tasks.
-- =============================================================================


CREATE OR REPLACE TASK TAA_DELTA_FINALIZE
    WAREHOUSE = &{{SNOWFLAKE_WAREHOUSE}}
    COMMENT = 'TAA delta load finalize: logs run summary after all 12 leaf tasks succeed.'
    AFTER
        TAA_DELTA_CUSTOMER
        ...
        TAA_DELTA_TIMEOFFREQUEST
        TAA_DELTA_TIMESLICEPOST
        ...
        TAA_DELTA_USERINFOPAYROLLMAPPING
        TAA_DELTA_LLDETAIL
        TAA_DELTA_USERINFOEMPSTATUS
AS
...
ALTER TASK TAA_DELTA_USERINFOPAYROLLMAPPING            RESUME;
ALTER TASK TAA_DELTA_LLDETAIL                          RESUME;
ALTER TASK TAA_DELTA_USERINFOEMPSTATUS                 RESUME;
ALTER TASK TAA_DELTA_ROOT                              RESUME;
